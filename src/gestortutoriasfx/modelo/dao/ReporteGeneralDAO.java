...
public class ReporteGeneralDAO {
    ...
    public static int insertarReporte(Connection c, ReporteGeneral r) throws SQLException {
        ...
        try (PreparedStatement ps = c.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            if (r.getIdProgramaEducativo() == null) ps.setNull(2, Types.INTEGER);
            else ps.setInt(2, r.getIdProgramaEducativo());

            if (r.getIdCoordinador() == null) ps.setNull(3, Types.INTEGER);
    ...
    public static int actualizarReporte(Connection c, ReporteGeneral r) throws SQLException {
        if (c == null) throw new SQLException("No hay conexión con la base de datos.");
        if (r.getIdReporteGeneral() == null) throw new SQLException("ID del reporte general es null.");
    ...
    public static ReporteGeneral obtenerPorId(Connection c, int idReporteGeneral) throws SQLException {
        ...
        String sql = "SELECT idReporteGeneral, idPeriodoEscolar, idProgramaEducativo, idCoordinador, " +
                     "nombreProgramaEducativo, nombreCoordinador, nombrePeriodoEscolar, " +
                     "numSesion, fecha, objetivos, totalAlumnosRegistrados, totalAlumnosAsistieron, " +
                     "porcentajeAsistencia, estatus, fechaGeneracion, estadoLugar " +
                     "FROM reporteGeneral WHERE idReporteGeneral=?";
    ...
    public static ArrayList<ReporteGeneral> listarPorCoordinador(Connection c, int idCoordinador, String busqueda) throws SQLException {
        ...
        String sql =
            "SELECT idReporteGeneral, idPeriodoEscolar, idProgramaEducativo, idCoordinador, " +
            "       nombreProgramaEducativo, nombreCoordinador, nombrePeriodoEscolar, " +
            "       numSesion, fecha, objetivos, totalAlumnosRegistrados, totalAlumnosAsistieron, " +
            "       porcentajeAsistencia, estatus, fechaGeneracion, estadoLugar " +
            "FROM reporteGeneral " +
    ...
    private static ReporteGeneral mapReporteGeneral(ResultSet rs) throws SQLException {
        ...
        r.setIdCoordinador((Integer) rs.getObject("idCoordinador"));
        ...
        if (estatus != null) {
            try { r.setEstatus(ReporteGeneral.Estatus.valueOf(estatus)); }
            catch (IllegalArgumentException ex) { r.setEstatus(ReporteGeneral.Estatus.Borrador); }
        }
